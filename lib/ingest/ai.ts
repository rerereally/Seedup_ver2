...
${input.context || '관련 자료를 찾지 못했다. 일반론을 최신 사실처럼 말하지 말고 불확실성을 표시하라.'}`,
    ...
    ], { models: getWritingModels(), jsonMode: true, temperature: 0, timeoutMs: 75_000, maxTokens: 5000 });
...
${input.context || '관련 자료 없음'}`,
      ...
      ], { models: getWritingModels(), temperature: 0, timeoutMs: 45_000, maxTokens: 5000 });
      return { evaluation: normalizeIdeaEvaluation(result, fallback), model };
    } catch (retryError) {
      console.error('OpenRouter idea evaluation retry failed', retryError);
      return { evaluation: null, model: null, error: '평가 모델이 응답하지 않았습니다. 잠시 후 다시 시도해주세요.' };
    }
  }
}
...
function normalizeIdeaEvaluation(result: Partial<IdeaEvaluation>, fallback: IdeaEvaluation): IdeaEvaluation {
  ...
  return {
    ...
    mvp_scope: normalizeMvpScope(result.mvp_scope, fallback.mvp_scope),
...
function normalizeMvpScope(value: unknown, fallback: EvaluationMvpScope | undefined): EvaluationMvpScope {
  ...
  return {
    ...
    core_user_flow: String(record.core_user_flow ?? fallback?.core_user_flow ?? '핵심 사용자 흐름 1개').trim(),
